...
def validate_ip(ip):
    try:
        ...
        raise UserInputError(f'ValueError: {ip} does not appear to be an IP address')
...
def port_init(args):
    ...
    if args.f=='tcp' and args.p is None:
        ...
        raise UserInputError('pmap.py: error: the following argument -p is mandatory for tcp test.')
...
def ip_init(args):
    ...
    if '-' in args.ip[0] and len(args.ip)>1:
        # print('usage: pmap.py [-h] -f {ping,tcp} -ip IP [-p P] [-n N] [-w W]')
        raise UserInputError('pmap.py: error: input the following argument -ip value in wrong format.')
...
if __name__=='__main__': 
    argparser = argparse.ArgumentParser(description='-'*25+'Help info of pmap tool'+'-'*25)
    argparser.add_argument('-f', type=str, required=True, choices=['ping','tcp'], help='"ping" for ping test; "tcp" for port connectinity test.')
    argparser.add_argument('-ip', type=str, nargs='+', required=True, help='ipV4 address, support single or patch in format "192.168.0.1-192.168.0.100"; "192.168.0.1 192.168.0.3".')
    argparser.add_argument('-p', type=int, nargs='*', help='port mandatory for tcp test, support single or patch in format "1-1024"(default); "80 443" .')
    argparser.add_argument('-m', type=str, choices=['proc', 'thread'], required=True, help='select to run by multiprocess or multithread.')
    argparser.add_argument('-n', type=int, choices=[2, 4, 8], help='Num of multiprocess.')
    argparser.add_argument('-w', action='store_true', help='Save the scan result to current folder.')
    argparser.add_argument('-v', action='store_true', help='Show the time spending for scanning.')
